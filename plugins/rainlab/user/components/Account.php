...
class Account extends ComponentBase
{
    ...
    public function onRegister()
    {
        ...
        try {
                                  ...
                                  //My additional code to the plugin      
    $userPhone = User::where('phone', post('phone'))->first();
if ($userPhone) {

    return ['#flash-div' => $this->renderPartial('verify')];
    ...
    public function onUpdateProfile()
    {
        if (!$user = $this->user()) {
         ...
         $data = post();
            if (!$user->checkHashValue('password', $data['password_current'])) {
                throw new ValidationException(['password_current' => 'Invalid Current Password']);
            }
        ...
        if (Input::hasFile('avatar')) {
         ...
         //added code to make background image(store_pic) work
         if (Input::hasFile('store_pic')) {
            $user->store_pic = Input::file('store_pic');
        }
